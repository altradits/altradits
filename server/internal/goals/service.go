...
func (s *Service) Create(ctx context.Context, input CreateInput) (*Goal, error) {
	...
	err := s.db.QueryRow(ctx, `
		INSERT INTO goals (name, emoji, target, deadline)
		VALUES ($1, $2, $3, $4)
		RETURNING
			id, name, emoji, target, saved,
			TO_CHAR(deadline, 'YYYY-MM-DD'),
			...
			TO_CHAR(completed_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"'),
			created_at
	`, input.Name, emoji, input.Target, deadline).
...
func (s *Service) Contribute(ctx context.Context, id string, amount float64) (*Goal, error) {
	var g Goal
	err := s.db.QueryRow(ctx, `
		UPDATE goals
		SET
